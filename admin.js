...
async function loadAiConfig(){try{const status=await api('ai-config');$('ai-status').textContent=status.configured?'Groq conectada. O botão Gerar descrição usa IA.':'Groq ainda não conectada. Use a chave gratuita para ativar descrições com IA.';$('ai-config-form').classList.toggle('hidden',status.configured);}catch{$('ai-status').textContent='Não foi possível consultar a configuração da IA.';}}
...
$('ai-config-form').onsubmit=async e=>{e.preventDefault();const button=$('ai-save');button.disabled=true;$('ai-status').textContent='Verificando chave na Groq...';try{await api('ai-config',{method:'POST',body:JSON.stringify({key:$('ai-key').value})});$('ai-key').value='';await loadAiConfig();message('IA conectada. Agora gere uma descrição para conferir.');}catch(error){$('ai-status').textContent='Groq não conectada.';message(error.message,true);}finally{button.disabled=false;}};
...
$('property-form').onsubmit=async e=>{e.preventDefault();const save=$('save');save.disabled=true;message('Preparando fotos...');try{if(!existingPhotos.length&&!selectedPhotos.length)throw Error('Adicione ao menos uma foto.');const photos=[];for(const file of selectedPhotos)photos.push(await resize(file));if(JSON.stringify(photos).length>15*1024*1024)throw Error('Fotos muito grandes; selecione menos fotos.');const fields=Object.fromEntries(new FormData(e.target).entries());await api('properties',{method:'POST',body:JSON.stringify({id:editing,fields,photos,removePhotos:removedPhotos})});reset();await load();message('Imóvel salvo e catálogo atualizado.');}catch(error){message(error.message,true);}finally{updatePublishState();}};
$('generate-description').onclick=async()=>{if($('descricao').value.trim()&&!confirm('Substituir a descrição atual pelo texto gerado?'))return;const button=$('generate-description');button.disabled=true;$('description-source').textContent='Gerando descrição...';try{const fields=Object.fromEntries(new FormData($('property-form')).entries());const result=await api('description',{method:'POST',body:JSON.stringify({fields})});$('descricao').value=result.description;updatePublishState();$('description-source').textContent=result.source==='ai'?'Texto gerado com IA (Groq). Revise antes de publicar.':'IA indisponível ou sem chave; texto automático usado. Revise antes de publicar.';message('Descrição pronta para revisão.');}catch(error){$('description-source').textContent='';message(error.message,true);}finally{button.disabled=false;}};
$('cancel').onclick=reset;
$('logout').onclick=async()=>{try{await api('logout',{method:'POST',body:'{}'});}finally{csrf='';$('ai-key').value='';clearMessage();reset();show(false);}};
