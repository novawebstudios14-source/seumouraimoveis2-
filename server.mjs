...
const fields=['titulo','finalidade','tipo','bairro','cidade','localizacao_mapa','preco','quartos','banheiros','area','descricao'];
const required=['titulo','finalidade','tipo','bairro','cidade','preco','quartos','banheiros','area','descricao'];
const labels={titulo:'Título',finalidade:'Finalidade (venda ou aluguel)',tipo:'Tipo',bairro:'Bairro',cidade:'Cidade',localizacao_mapa:'Localização no Google Maps',preco:'Preço em reais',quartos:'Quartos',banheiros:'Banheiros',area:'Área em m²',descricao:'Descrição'};
...
async function handler(req,res){try{
  ...
  if(url.pathname.startsWith('/api/admin/')){
    ...
    if(url.pathname==='/api/admin/properties'&&req.method==='POST'){
      ...
      const f={};for(const key of fields)f[key]=safeText(input.fields[key],key==='descricao'?1000:key==='localizacao_mapa'?300:160);
