...
function addProperty(property){
  ...
  const cover=document.createElement('a');cover.href=detailUrl;cover.setAttribute('aria-label','Ver detalhes de '+property.titulo);cover.className='catalog-cover-link';const img=document.createElement('img');img.src=property.photos[0];img.alt='Foto de '+property.titulo;img.loading='lazy';cover.append(img);media.append(cover);
  ...
  const details=document.createElement('a');details.href=detailUrl;details.className='catalog-details-link';details.textContent='Ver detalhes do imóvel →';
  body.append(heading,specs,price,details,link);article.append(media,body);
  if(property.photos.length>1){const gallery=document.createElement('div');gallery.className='live-gallery';gallery.setAttribute('aria-label','Outras fotos do imóvel');for(const [i,src] of property.photos.entries()){if(i===0)continue;const a=document.createElement('a');a.href=src;a.target='_blank';a.rel='noopener noreferrer';a.setAttribute('aria-label',`Abrir foto ${i+1} de ${property.titulo}`);const thumb=document.createElement('img');thumb.src=src;thumb.alt='';thumb.loading='lazy';a.append(thumb);gallery.append(a);}body.append(gallery);}
  grid.append(article);
}
